...
namespace MKW.Domain.Dto.DTO.EmailDTO
{
    ...
    {
        public MessageRecoveryPasswordDTO(IEnumerable<string> sendToList, string keycode, string language = "pt-BR")
        {
            ...
            Subject = EmailLangHelper.GetString("ResetPasswordSubject") ?? "Recuperacao de Senha";
            Content = $"" +
                        ...
                        $"<tr>" +
                            $"<td bgcolor=\"#ffffff\" align=\"center\" style=\"padding: 40px 0;\">" +
                                $"<h1 style=\"font-size: 40px; font-weight: bold; margin: 0; color: #016FB9;\">" +
                                    $"MyKidsWatch<br>" +
                                    $"{EmailLangHelper.GetString("ResetPasswordTitle") ?? "Recuperacão de Senha"}<br> </h1>" +
                            $"</td>" +
                        $"</tr>" +
                        $"<tr>" +
                            $"<td bgcolor=\"#ffffff\" align=\"center\" style=\"padding: 20px 0;\">" +
                                $"<div style=\"max-width: 400px; margin: 0 auto;\">" +
                                    ...
                                    $"<h2 style=\"font-size: 24px; font-weight: bold; margin: 0; color: #ffffff;\">{EmailLangHelper.GetString("ResetPasswordBoxTitle") ?? "Código de Recuperação"}</h2>" +
                                        $"<p style=\"font-size: 18px; margin: 20px 0; color: #ffffff;\">{EmailLangHelper.GetString("ResetPasswordBoxSubtitle") ?? "Utilize o código abaixo para recuperar sua senha:"}</p>" +
